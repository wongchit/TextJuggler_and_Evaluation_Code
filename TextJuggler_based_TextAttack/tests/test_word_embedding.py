...
from textattack.models import helpers
...
def test_embedding_paragramcf():
    ...
    lstm = helpers.LSTMForClassification()
    glove_tokenizer = lstm.tokenizer
    token = glove_tokenizer.encode(raw_text)
    seq=[]
    for word in glove_tokenizer.convert_ids_to_tokens(token):
        seq.append(word_embedding.embedding_matrix[word_embedding.word2index(word)])
    print(seq)'''
